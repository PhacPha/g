...
class LogoController {
    ...
    public function __construct() {
        ...
        // ตรวจสอบและสร้างโฟลเดอร์ img หากยังไม่มี (กำหนดใน config.php แล้ว)
    }
    ...
    public function manage() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit_logo') {
            ...
            if ($image_url) {
                // ลบรูปภาพเก่าถ้ามี
                if ($old_logo['image_url'] && file_exists(IMAGE_UPLOAD_DIR . basename($old_logo['image_url']))) {
                    unlink(IMAGE_UPLOAD_DIR . basename($old_logo['image_url']));
                }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action']) && $_GET['action'] === 'delete_logo') {
            ...
            if ($logo['image_url'] && file_exists(IMAGE_UPLOAD_DIR . basename($logo['image_url']))) {
                unlink(IMAGE_UPLOAD_DIR . basename($logo['image_url']));
            }
    ...
    private function handleImageUpload($fileInputName, $urlInput) {
        ...
        if (isset($_FILES[$fileInputName]) && $_FILES[$fileInputName]['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($file['tmp_name'], $uploadPath)) {
                return IMAGE_UPLOAD_PATH . $fileName; // คืนค่า path สำหรับบันทึกในฐานข้อมูล
            } else {
                return false;
            }
        }
